  ...
  render() {
    return (
      <ScrollView contentContainerStyle={GlobalStyle.container}>
        ...
        <TouchableOpacity style={[GlobalStyle.pillButtonSide, GlobalStyle.shadow, { top: 30 }]} onPress={() => this.handleLogin(this.state.email, this.state.password)}>
          <Text style={GlobalStyle.pillButtonSideText} > Sign In </Text>
        </TouchableOpacity>
        <TouchableOpacity>
          <Text style={[GlobalStyle.text, { top: Constant.MAX_HEIGHT / 4 }]} onPress={() => this.props.navigation.navigate('SignUp')}> Create an Account </Text>
        </TouchableOpacity>
        {this.state.iserror ?
